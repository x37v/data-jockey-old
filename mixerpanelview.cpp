...
: QWidget(parent)
{
	...
	for(unsigned int i = 0; i < numMixers; i++){
		...
		mLayout->addWidget(cur->DJMixerControl(), 0, i, Qt::AlignCenter);
		mLayout->addWidget(cur->mixerChannel()->eq(), 1, i, Qt::AlignCenter);
		mLayout->addWidget(cur->mixerChannel()->muteButton(), 2, i, Qt::AlignCenter);
		mLayout->addWidget(cur->mixerChannel()->volumeSlider(), 3, i, Qt::AlignHCenter);
		mLayout->setColumnStretch(i,0);
	}

	//add the master widgets and the cross fade
	mLayout->addWidget(mMaster->volume(), 3, numMixers, Qt::AlignHCenter);
